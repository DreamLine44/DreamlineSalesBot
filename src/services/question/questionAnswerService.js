...
// [FIX-HOURS-NATURAL] Customers rarely say "opening hours" â€” they ask "are
...
// costs" â€” the word-boundary after "price"/"cost" fails when an "s" follows
...
// "wave"/"cash"/"mobile money" â€” "accept ... card" phrasing fell through.
...
export const formatMenuText = (business, { maxItems = 30, heading = null } = {}) => {
  ...
  const items = isSalon && services.length
    ? services.map(s => ({ name: s.name, price: s.price, emoji: 'ðŸ’‡' }))
    : menuItems.map(i => ({ name: i.name, price: i.price, emoji: 'ðŸ½ï¸' }));
  ...
  const lines = shown.map(i => {
    const priceStr = i.price != null ? ` â€” ${currency}${formatMoney(i.price)}` : '';
    return `${i.emoji} *${i.name}*${priceStr}`;
  });

  const title = heading || (isSalon ? 'ðŸ’‡ Our Services' : 'ðŸ½ï¸ Today\'s Menu');
  let body = `${title}\n\n${lines.join('\n')}`;
...
export const formatHoursText = (business) => {
  ...
  if (!hours?.enabled) {
    return business?.adminPhone
      ? `Please contact us at ðŸ“ž *${business.adminPhone}* for our opening hours.`
  ...
  if (!lines.length) {
    ...
    if (open && close) return `ðŸ• *Opening Hours*\n\n${open} â€“ ${close}`;
...
export async function tryDatabaseAnswer({ message, business, session }) {
  ...
  if (qType === 'AVAILABILITY') {
    ...
    if (candidates.length === 1) {
      return { handled: true, body: `âœ… Yes, *${candidates[0].name}* is currently available.`, routingDecision: 'QUESTION', context: { lastMessage: raw, lastTopic: 'AVAILABILITY' } };
    }
    if (candidates.length > 1) {
      return { handled: true, body: `Which item do you mean â€” ${candidates.slice(0, 4).map(item => `*${item.name}*`).join(', ')}?`, routingDecision: 'QUESTION', context: { lastMessage: raw, lastTopic: 'AVAILABILITY' } };
    }
  ...
  if (qType === 'PRICE') {
    ...
    if (candidates.length > 1) {
      return { handled: true, body: `Which one do you mean â€” ${candidates.slice(0, 4).map(item => `*${item.name}*`).join(', ')}?`, routingDecision: 'QUESTION', context: { lastMessage: raw, lastTopic: 'PRICE' } };
    }
    ...
    // tried to parse it as a product name and reported a catalogue miss â€”
    ...
    if (menu.length) {
      const menuText = formatMenuText(business, { heading: 'ðŸ’° Prices' });
      return {
        handled: true,
  ...
  if (qType === 'ADDRESS' || qType === 'CONTACT') {
    ...
    const parts = [];
    if (qType === 'ADDRESS' && addr) parts.push(`ðŸ“ *Location*\n\n${addr}`);
...
export async function processQuestionMessage({ session, message, business, tenant, intent = 'FAQ' }) {
  ...
  if (!isBusinessScopeQuestion(raw, business)) {
    return {
      type: 'text',
      body: "I'm here to help with questions about our business â€” menu, services, hours, orders, and bookings. What would you like to know?",
