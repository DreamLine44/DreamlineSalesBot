...
// [RESTORE-CANCEL-CONFIRMED-GUARD] status:'confirmed' (and everything after it in the
// kitchen pipeline) is the authoritative "order accepted by admin" signal in this
// codebase. Once an order reaches one of these statuses it must NOT be
// self-cancellable by the customer — only a truly 'pending' order (never accepted)
// or one still awaiting the customer's action after a rejected/unpaid payment is.
// This mirrors the merged-in local build's [AUDIT-FIX-CANCEL-CONFIRMED-GUARD] /
// [FIX-CANCEL-REJECTED] guards, which the refactor into this shared service had
// accidentally widened (IN_PROGRESS_ORDER_STATUSES included 'confirmed'/'preparing'/
// 'ready'/'out_for_delivery' in the *cancellable* filter, not just the *visible/active*
// one) — that would have let a customer cancel an order the kitchen was already
// preparing. See buildCustomerCancellableOrderFilter and _cancelActivityByReference.
const ADMIN_ACCEPTED_ORDER_STATUSES = ['confirmed', 'preparing', 'ready', 'out_for_delivery'];
...
export function buildCustomerCancellableOrderFilter(customerPhone, tenantId) {
  ...
  return {
    customerPhone,
    tenantId,
    // [RESTORE-CANCEL-CONFIRMED-GUARD] Exclude admin-accepted orders outright —
    // these must go through support, not customer self-cancel.
    status: { $nin: [...TERMINAL_ORDER_STATUSES, ...ADMIN_ACCEPTED_ORDER_STATUSES] },
    $or: [
      { status: 'pending', paymentStatus: 'unpaid', paymentReviewedAt: { $ne: null } },
      {
        status: 'pending',
...
async function _cancelActivityByReference({ ref, customerPhone, tenantId, business }) {
  ...
  if (order) {
    ...
    // [RESTORE-CANCEL-CONFIRMED-GUARD] Same guard as the bare-cancel path above —
    // once an admin has accepted the order it's no longer customer self-cancellable,
    // reference or not.
    if (ADMIN_ACCEPTED_ORDER_STATUSES.includes(order.status)) {
      return {
        type: 'text',
        body: `⚠️ Order *#${ref}* has already been confirmed and is being prepared, so it can't be self-cancelled at this stage.\n\nPlease contact us directly if you still need to cancel.`,
      };
    }
