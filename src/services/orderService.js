...
// loop, or a caller bug) — the multiItemCart.maxItems config field is
// enforced at the WA Catalog cart entry point (waCatalogFlow.js), but no
// per-vertical chat flow has an in-chat "add another item" loop yet — WA
// Catalog's native cart UI is currently the only way a customer builds a
// multi-item order. This cap protects saveOrder()/the Order schema either
// way, whichever caller reaches it.
//
// resolveOrderFields() IS wired into saveOrder() below, and items[] IS
// persisted on the Order document (see models/Order.js) — both were
// completed in a later pass; this comment previously said otherwise.
