 ...
 *   APPROVE <shortId>                 â€” approve payment proof
 *   REJECT <shortId>                  â€” reject payment proof
 *   CONFIRM BOOK <shortId>            â€” confirm a booking
 *   DECLINE BOOK <shortId> [reason]   â€” decline a booking
 *   RESUME BOT <phone>                â€” exit human handoff mode
 *
 ...
 *               â€” extremely long malformed IDs now return null instead of traversing
 ...
 *               doesn't match any pattern â€” previously returned null and the message
 ...
 *               PAYMENT_PROOF step â€” previously fire-and-forget meant a transient DB
 ...
 * [FIX-CMD-11]  confirmPayment() and rejectPayment() $or query cleaned up â€” previously included
 ...
 *               all isAdminPhone call sites, reducing admin-path DB reads from 3 â†’ 0.
 *               getModeConfig moved to a static top-level import â€” previously it was
 ...
 *               then immediately overwrites it to 'cancelled' â€” the AWAIT_ADMIN_CONFIRM
 ...
 *               produced ZERO response â€” no success message, no error, nothing. Now every
 ...
 * [FIX-CMD-16]  Removed buildAdminBookingAlert() â€” dead code. It was exported but never
...
// â”€â”€ Admin phone check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// passed in as a parameter) and forwards them here â€” eliminating both DB queries
...
export async function isAdminPhone(senderPhone, tenantId, business = null, tenantDoc = null) {
  ...
  // 1. Fast env-var check (no DB) â€” check first
...
// â”€â”€ Admin button reply â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function handleAdminButtonReply(buttonId, tenantId, adminPhone, tenantDoc, business) {
  ...
  // [FIX-X3] RESUME_BOT_<phone> button â€” dispatched by the support escalation alert
...
// â”€â”€ Admin text command router â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function handleAdminTextCommand(text, tenantId, adminPhone, tenantDoc, business) {
  ...
  // [SPEC-5A] MARK READY <shortId> â€” notify customer their order is ready for collection
  ...
  if (cancelAnyMatch) {
    ...
    return `âš ï¸ No order or booking found: #${id}`;
  }
  ...
  // [FIX-HM-3] "RESUME BOT" with NO phone number â€” find the most recent human-mode
  ...
  if (upper === 'RESUME BOT') {
    ...
    if (latest?.customerPhone) {
      ...
      if (remaining > 0) {
        ...
        // backtick `RESUME BOT <phone>` commands the admin had to read and re-type â€”
        ...
        // same one-tap pattern: â‰¤3 remaining customers get inline buttons, more than
        ...
        // webhookController â€” no typing required either way.
        const introText =
          resumeReply +
          `\n\nâš ï¸ *${remaining} other customer${remaining > 1 ? 's are' : ' is'} still in human-mode.*\n` +
        ...
        if (remainingSessions.length <= 3) {
          return {
            ...
            buttons: remainingSessions.map(s => ({
              ...
              title: `â–¶ï¸ ${s.customerPhone}`.slice(0, 20),
            })),
          };
        }
        return {
    ...
    return `â„¹ï¸ No active human-mode sessions found for this business.
...
Use \`RESUME BOT <phone>\` to resume a specific customer.`;
  ...
  if (looksLikeCommand) {
    return (
      `âš ï¸ *Unrecognised command format.*\n\n` +
      `Valid admin commands:\n` +
      `âœ… \`APPROVE <shortId>\`\n` +
      `âŒ \`REJECT <shortId>\`\n` +
      `ðŸ›‘ \`CANCEL ORDER #<shortId>\`\n` +
      `ðŸ›‘ \`CANCEL BOOKING #<shortId>\`\n` +
      `ðŸ›‘ \`CANCEL #<shortId>\`\n` +
      `ðŸ“… \`CONFIRM BOOK <shortId>\`\n` +
      `ðŸš« \`DECLINE BOOK <shortId> [reason]\`\n` +
      `ðŸ½ï¸ \`MARK READY <shortId>\` _(notify customer to collect)_\n` +
      `ðŸ¤– \`RESUME BOT <phone>\`\n` +
      `ðŸ¤– \`RESUME BOT\` _(resumes most recent human-mode session)_`
    );
  }
...
// â”€â”€ Confirm payment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function confirmPayment(shortId, tenantId, adminPhone, tenantDoc, business) {
  try {
    ...
    if (!order) {
      ...
      if (!existing) return `âš ï¸ No order found: ${shortId}`;
      if (existing.paymentStatus === 'confirmed') return `â„¹ï¸ Order #${shortId} already confirmed.`;
      return `âš ï¸ Order #${shortId} can't be confirmed â€” current status is *${existing.status}*.`;
    }
    ...
    // which expires with the session TTL (~30 min) â€” so an admin confirming a cash
    ...
    // [FIX-CONFIRM-BTN] Plain text only on the initial confirmation â€” no welcome buttons.
    ...
    //   - Thank-you sign-off
    //   - NO buttons (state â†’ PREPARING, customer should wait)
    const bizName = business?.name || 'us';
    // [FIX-ETA] Use business.settings.estimatedDeliveryMinutes if configured (mirrors postFlowHandler
    // [PFH-3]). Previously hardcoded "20â€“30 minutes" â€” wrong for bakeries, salons, retail etc.
    const etaMins = business?.settings?.estimatedDeliveryMinutes;
    // [CLEANUP] etaLine removed â€” superseded by etaLineToShow below (FIX-SALON-18), was dead code.
    // [FIX-SALON-18] Mode-aware order confirmation message. Previously hardcoded
    // "ðŸ³ Our kitchen is now preparing your order" for ALL business modes â€” wrong
    ...
    // ETA fallback "20-30 minutes" for modes where it's meaningless â€” instead show
    ...
    const preparingLine = _isFoodMode
      ? `ðŸ³ Our kitchen is now preparing your order.\n`
      : `ðŸ›ï¸  We're preparing your order.\n`;
    const etaLineToShow = etaMins
      ? `â±ï¸  Estimated time: ${etaMins} minutes.\n\n`
      : (_isFoodMode ? `â±ï¸  Estimated time: 20â€“30 minutes.\n\n` : `\n`);

    const confirmBody = isCashConfirm
      ? `âœ… *Order Confirmed!*\n\n` +
        `Your order has been accepted.\n\n` +
        `${itemsBlock}\n` +
        `ðŸ”–  Reference: #${order.shortId || shortId}\n\n` +
        preparingLine +
        etaLineToShow +
        `We'll message you the moment it's ready.\n\n` +
        `Thank you for choosing *${bizName}* ðŸ˜Š`
    ...
    // [PFH-5 / MEM-FIX-1] Record confirmed order in customer memory â€” only fires on
    ...
    // saveOrder() succeeded â€” i.e. before any payment was verified. An order that was
    ...
    // double-counting class already fixed for stats.totalOrders (see MEM-FIX-1 above) â€”
    ...
    // applied to revenue. Moving the single recordRevenue() call here â€” the one place
    // that runs only once, only on an admin-approved payment (proof-verified or cash
    // self-confirm both funnel through this function) â€” makes revenue reflect money
    ...
    // Previously the admin got "âœ… Payment confirmed" as plain text with no next action.
    // They had to remember to type "MARK READY <shortId>" later â€” a step many admins
    ...
    return `âš ï¸ Something went wrong confirming order #${shortId}. Please try again.`;
  }
}

// â”€â”€ Reject payment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  try {
    ...
    if (!order) {
      const existing = await Order.findOne({ shortId, tenantId })
        .select('status paymentStatus').lean().catch(() => null);
      if (!existing) return `âš ï¸ No order found: ${shortId}`;
      if (existing.status === 'cancelled') return `â„¹ï¸ Order #${shortId} already cancelled.`;
      return `âš ï¸ Order #${shortId} can't be rejected â€” payment is already confirmed.`;
    }

    // [FIX-AWAIT] Distinguish cash orders (AWAIT_ADMIN_CONFIRM) from payment-proof orders.
    // Cash orders have no screenshot to retry â€” rejection means the order is cancelled
    ...
    // overwrote those same fields to 'cancelled' in the cash branch â€” a wasted write
    ...
    if (isCashOrder) {
      // Cash/delivery rejection â€” mark cancelled, clear session, let customer restart
      ...
      if (!updated) return `â„¹ï¸ Order #${shortId} was already handled.`;
      ...
      // { type: 'buttons', buttons: modeCfg.ui?.welcomeButtons } reply â€” the
      // exact bug class core/shared/uiOptionsHelper.js's [FIX-EXPOSED-BUTTONS-1]
      // fixed everywhere else (buildWelcomeSequence's "Choose an option â–¼"
      // dropdown getting bypassed by the raw "Order Food / Book a Table / â‹¯ More"
      // 3-button layout on secondary prompts). This admin-initiated cash-order
      // cancellation notice was missed by that earlier audit â€” the customer-
      ...
      return `âŒ *Order cancelled*\n\nOrder #${shortId} â€” ${order.item}\nCustomer ${order.customerPhone} notified.`;
    }

    // [FIX-CMD-2] Reset BOTH paymentStatus AND status consistently:
    // paymentStatus='unpaid'  â†’ receiveProof() will accept a new screenshot
    // status='pending'        â†’ order is alive and awaiting retry (was 'payment_failed',
    ...
    const updated = await Order.findOneAndUpdate(guardFilter, { $set: {
      ...
      // can surface it to the customer. Previously only postFlowData carried the reason â€”
    ...
    // [FIX-CMD-8] Await the session update â€” if this fails the customer's session won't
    ...
    await dispatchMessage(order.customerPhone, {
      type:    'buttons',
      body:
        `âŒ *Payment Verification Failed*\n\n` +
        `We could not verify your payment for order *#${order.shortId || shortId}*.\n\n` +
        `*Possible reasons:*\n` +
        `â€¢ Incorrect amount sent\n` +
        `â€¢ Payment sent to the wrong account\n` +
        `â€¢ Screenshot was unclear or incomplete\n\n` +
    ...
    // [FIX-CMD-2] Log dispatch failures â€” customer won't know to retry if this silently fails.
    ...
    return `âŒ *Payment rejected*\n\nOrder #${shortId} â€” ${order.item}\nCustomer ${order.customerPhone} notified. Retry window open.`;
  } catch (err) {
    // [FIX-CMD-15] See confirmPayment â€” guarantees the admin always gets a reply
    ...
    return `âš ï¸ Something went wrong rejecting order #${shortId}. Please try again.`;
  }
}

// â”€â”€ Confirm booking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function confirmBooking(shortId, tenantId, adminPhone, tenantDoc) {
  try {
    ...
    // confirmPayment â€” two near-simultaneous CONFIRM BOOK taps could previously both
    ...
    if (!booking) {
      ...
      if (!existing) return `âš ï¸ No booking found: ${shortId}`;
      return `â„¹ï¸ Booking #${shortId} already confirmed.`;
    }

    const when       = booking.time ? `${booking.date} at ${booking.time}` : booking.date;
    const serviceStr = booking.service ? ` (${booking.service})` : '';

    // [SPEC-6C] Booking confirmed message â€” structured format with reference,
    ...
    const bookingBody =
      `âœ… *${isWalkInBooking ? 'Walk-In Queue Confirmed!' : 'Booking Confirmed!'}*${nameForBody}\n\n` +
      (!isWalkInBooking && booking.date ? `ðŸ“…  Date: *${booking.date}*\n` : '') +
      (!isWalkInBooking && booking.time ? `â°  Time: *${booking.time}*\n` : '') +
      (booking.partySize ? `ðŸ‘¥  Party size: *${booking.partySize}*\n` : '') +
      (booking.service   ? `ðŸ’‡  Service: *${booking.service}*\n`       : '') +
      (booking.staff     ? `ðŸ‘¤  Stylist/Barber: *${booking.staff}*\n`   : '') +
      `ðŸ”–  Reference: *${shortId}*\n\n` +
    ...
    await dispatchMessage(booking.customerPhone, {
      ...
      buttons: isWalkInBooking
        ? [
            { id: 'QUESTION',       title: 'â“ Ask a Question' },
            { id: 'CANCEL_BOOKING', title: 'âŒ Leave Queue'     },
          ]
        : [
    ...
    // [FIX-CMD-2] Log dispatch failures â€” customer won't know their booking is confirmed.
        ...
        if (_retailItems.length > 0) {
          ...
          if (_upsellItems.length > 0) {
            const _productLines = _upsellItems.map(p => {
              const _price = p.price ? ` â€” ${p.currency || _currency}${formatMoney(p.price)}` : '';
              return `â€¢ *${p.name}*${_price}`;
            }).join('\n');

            const _isBarbershopUpsell = _mode === 'BARBERSHOP';
            const _upsellBody =
              `${_isBarbershopUpsell ? 'âœ‚ï¸' : 'ðŸ’‡'} *Aftercare tip:* Maintain your results at home!\n\n` +
              `We recommend:\n${_productLines}\n\n` +
    ...
    return `âš ï¸ Something went wrong confirming booking #${shortId}. Please try again.`;
  }
}

// â”€â”€ Decline booking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function declineBooking(shortId, reason, tenantId, adminPhone, tenantDoc) {
  try {
    // [FIX-CMD-14] Atomic findOneAndUpdate â€” same TOCTOU rationale as confirmBooking.
    const booking = await Booking.findOneAndUpdate(
    ...
    if (!booking) {
      const existing = await Booking.findOne({ shortId: shortId.toUpperCase(), tenantId })
        .select('status').lean().catch(() => null);
      if (!existing) return `âš ï¸ No booking found: ${shortId}`;
      return `â„¹ï¸ Booking #${shortId} already cancelled.`;
    }
    ...
    // Walk-in: "we can't add you to the queue right now" â€” no "arrange alternative time"
    ...
    await dispatchMessage(booking.customerPhone, {
      ...
      buttons: isWalkInDec
        ...
        : [
            { id: 'BOOK',     title: 'ðŸ“… Try Different Date' },
            { id: 'QUESTION', title: 'â“ Ask a Question'      },
          ],
    },
    // [FIX-CMD-2] Log dispatch failures â€” customer won't know their booking was declined.
    ...
    return `âŒ *Booking declined*\n\nBooking #${shortId} â€” ${when}${serviceStr}${reason ? `\nReason: ${reason}` : ''}\nCustomer ${booking.customerPhone} notified.`;
  } catch (err) {
    // [FIX-CMD-15] Guarantee the admin always gets a reply instead of silent failure.
    logger.error('[AdminCmd] declineBooking failed', { shortId, adminPhone, err: err.message });
    return `âš ï¸ Something went wrong declining booking #${shortId}. Please try again.`;
  }
}

// â”€â”€ Cancel order by reference (admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function cancelOrderByShortId(shortId, tenantId, adminPhone, tenantDoc, business) {
  try {
    ...
    if (!order) {
      ...
      if (!existing) return `âš ï¸ No order found: #${ref}`;
      if (terminal.includes(existing.status)) {
        return `â„¹ï¸ Order #${ref} is already *${existing.status}* and cannot be cancelled.`;
      }
      return `âš ï¸ Order #${ref} could not be cancelled. Please try again.`;
    }
    ...
    return `ðŸ›‘ *Order cancelled*\n\nOrder #${ref} â€” customer ${order.customerPhone} has been notified.`;
  } catch (err) {
    logger.error('[AdminCmd] cancelOrderByShortId failed', { shortId, adminPhone, err: err.message });
    return `âš ï¸ Something went wrong cancelling order #${shortId}. Please try again.`;
  }
}

// â”€â”€ Cancel booking by reference (admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function cancelBookingByShortId(shortId, tenantId, adminPhone, tenantDoc) {
  try {
    ...
    if (!booking) {
      ...
      if (!existing) return `âš ï¸ No booking found: #${ref}`;
      if (existing.status === 'cancelled') return `â„¹ï¸ Booking #${ref} is already cancelled.`;
      if (existing.status === 'completed') return `â„¹ï¸ Booking #${ref} is already completed and cannot be cancelled.`;
      return `âš ï¸ Booking #${ref} could not be cancelled. Please try again.`;
    }
    ...
    return `ðŸ›‘ *Booking cancelled*\n\nBooking #${ref}${serviceStr} â€” customer ${booking.customerPhone} has been notified.`;
  } catch (err) {
    logger.error('[AdminCmd] cancelBookingByShortId failed', { shortId, adminPhone, err: err.message });
    return `âš ï¸ Something went wrong cancelling booking #${shortId}. Please try again.`;
  }
}

// â”€â”€ Mark order ready â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// [SPEC-5A] Admin types "MARK READY <shortId>" or taps READY_<shortId> button.
// Updates order status to 'ready', dispatches the spec Â§5A message to the customer.
async function markOrderReady(shortId, tenantId, adminPhone, tenantDoc, business) {
  try {
    ...
    if (!order) {
      ...
      if (!existing)                      return `âš ï¸ No order found: ${shortId}`;
      if (existing.status === 'ready')     return `â„¹ï¸ Order #${shortId} is already marked ready.`;
      if (existing.status === 'completed') return `â„¹ï¸ Order #${shortId} is already completed.`;
      return `âš ï¸ Order #${shortId} can't be marked ready â€” status is *${existing.status}*.`;
    }
    ...
    await dispatchMessage(order.customerPhone, {
      ...
      buttons: [
        { id: `COLLECTED_${order.shortId || shortId}`, title: 'âœ… Collected â€” Thanks!' },
    ...
    return `âœ… *Order ready notification sent*\n\nOrder #${shortId} â€” ${order.item}\nCustomer ${order.customerPhone} notified to collect.`;
  } catch (err) {
    logger.error('[AdminCmd] markOrderReady failed', { shortId, adminPhone, err: err.message });
    return `âš ï¸ Something went wrong marking order #${shortId} ready. Please try again.`;
  }
}

// â”€â”€ Resume bot â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function resumeBot(customerPhone, tenantId, tenantDoc) {
  // upsert returns null when no document matches â€” meaning there is no active
  ...
  if (!updated) {
    logger.warn('[AdminCmd] resumeBot: no active session found for customer â€” TTL may have expired', {
  ...
  // exists in the DB â€” firing a "bot is back!" message for a conversation that is gone
  ...
  if (updated && tenantDoc) {
    dispatchMessage(
      ...
      {
        type:    'buttons',
        body:    `âœ… Our team has finished assisting you. Our automated assistant is back! ðŸ˜Š\n\nTap below if you need anything.`,
        buttons: [
          { id: 'SHOW_MENU', title: 'ðŸ”„ Main Menu'       },
          { id: 'QUESTION',  title: 'â“ Ask a Question'  },
        ],
  ...
  return `âœ… Bot resumed for *${customerPhone}*. Automation is active again.`;
}

// â”€â”€ Admin alert builders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// [FIX-CMD-16] buildAdminBookingAlert() (the version with a CONFIRM BOOK/DECLINE BOOK
// text-command footer baked into the body) was removed as dead code â€” it was exported
...
export const buildAdminBookingAlertBody = ({ customerPhone, date, time, service, partySize, business, shortId, staff, bookingType }) => {
  ...
  const typeHeader    = isWalkIn ? 'ðŸš¶ *Walk-In Queue* â€” ' : 'ðŸ“… *New Booking* â€” ';
  ...
  const walkInTimeStr = isWalkIn
    ? `\nâ° Joined: *${new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZone: _walkInTz })}*`
    : '';

  const dateStr = (!isWalkIn && date) ? `\nðŸ“… Date: *${date}*` : '';

  return (
    `ðŸ”” ${typeHeader}${bizName}\n\n` +
    ...
    partyStr +
    idStr +
    '\n\nâ³ Status: *Pending* â€” please confirm or decline.'
  );
}
