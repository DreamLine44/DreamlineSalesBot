...
export const maybeAppendExpressionClosing = (body, flowData) => {
...
export const formatExpressionReply = (body, flowData) => {
  return maybeAppendExpressionClosing(trimExpressionReply(body), flowData);
}

export const getExpressionTurnsLeft = (flowData) => {
  const n = flowData?._exprTurnsLeft;
  if (typeof n === 'number' && n >= 0) return n;
  return EXPRESSION_TURN_BUDGET;
}

export const isExpressionSentiment = (sentiment) => {
...
export const shouldHandleAsPostFlowExpression = (msg, sentiment) => {
...
export const buildExpressionSessionContext = ({ business, subType, lastBotReply, lastCustomerMsg, customerMessage }) => {
  const parts = [
...
export const preserveExpressionTurns = (flowData = {}) => {
