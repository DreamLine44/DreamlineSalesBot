...
test('withCatalogWelcomeOption paginates into a "⋯ More" screen (never silently drops a button, never a 4th button) when it would exceed 3', () => {
  const buttons = [
    { id: 'ORDER', title: '🛍 Shop' },
    { id: 'BOOK', title: '📅 Book' },
  ...
  const result = withCatalogWelcomeOption(buttons, makeBusiness());
  assert.equal(result.buttons.length, 3, 'primary screen must never exceed the 3-button cap');
  assert.deepEqual(result.buttons.map(b => b.id), ['ORDER', 'BOOK', 'MORE_MENU']);
  assert.equal(result.more.buttons.length, 3, '"more" screen must also never exceed the 3-button cap');
  assert.deepEqual(result.more.buttons.map(b => b.id), ['QUESTION', 'BROWSE_CATALOG', 'SHOW_MENU']);
});
