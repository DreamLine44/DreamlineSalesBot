...
test('webhookController.js: detectIntent destructure includes aiSignals', () => {
  const src = read('../controllers/webhookController.js');
  assert.ok(
    // [MERGE-FIX-1] Relaxed from an exact "suggestion, aiSignals }" match to allow
    // additional destructured fields (e.g. `hesitant`, added alongside aiSignals
    // for the negation/emotion-guard merge) — the test's actual intent, per its
    // own assertion message, is just that aiSignals is destructured at this call
    // site, not that it's the literal last field. Mirrors the flexible pattern
    // already used by the neighboring 'imports isInformationalIntent' test above.
    /const\s*\{[^}]*\baction\b[^}]*\bintent\b[^}]*\bconfidence\b[^}]*\bsuggestion\b[^}]*\baiSignals\b[^}]*\}\s*=\s*await\s*detectIntent\(/.test(src),
