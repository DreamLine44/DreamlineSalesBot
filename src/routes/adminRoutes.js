...
// [ADMIN-NOTIFY-1] Pure helpers for the super-admin ↔ tenant-admin messaging
// routes. Kept dependency-free (no Mongo, no req/res) so they're cheaply
// unit-testable — see tests/adminNotifications.test.mjs.
const NOTIFICATION_SEVERITIES = ['info', 'warning', 'success', 'error'];

/** Validates a notification payload before it's persisted. Returns an error
...
/** Builds the role-scoped Mongo filter for reading admin↔tenant notification
 *  threads. SECURITY-critical: a tenant-admin caller must never be able to
 *  read another tenant's thread or enumerate broadcasts by ID, even via a
 *  forged query param. */
export function buildNotificationAccessFilter(admin = {}, query = {}) {
  const { isSuperAdmin, tenantId: adminTenantId } = admin;
  if (!isSuperAdmin && !adminTenantId) return { error: 'Forbidden' };
  ...
  if (isSuperAdmin) {
    ...
    // Tenant callers are hard-pinned to their own tenantId — query.tenantId
    // is deliberately ignored. broadcastId is also withheld from tenant
    // callers to prevent enumerating other tenants' broadcast threads.
    filter.tenantId = adminTenantId;
  }

  const VALID_DIRECTIONS = ['TO_ADMIN', 'TO_TENANT'];
  if (VALID_DIRECTIONS.includes(query.direction)) filter.direction = query.direction;
