...
export async function route({ action, intent, session, message, business, tenant, isInteractive, suggestion, nlu }) {
  ...
  switch (upper) {
    ...
    case 'BROWSE_CATALOG': {
      // [AUDIT-FIX-CATALOG-DIRECT] Per explicit request, the intermediate
      // "Here's how to see what we have — tap below!" / "🛍 View Items"
      // confirmation step (previously gated on !isInteractive) has been
      // removed. Both a typed menu request and an interactive button tap
      // now go straight to browseCatalogExplicit(), which sends the real
      // catalog message directly and already handles its own fallback to
      // the text/list ORDER menu when WA Catalog isn't configured/enabled
      // for the tenant.
      const { browseCatalogExplicit } = await import('../../modules/catalog/waCatalogFlow.js');
      return browseCatalogExplicit({ session, business, tenant });
    }
