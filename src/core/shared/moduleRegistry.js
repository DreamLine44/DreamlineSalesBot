...
// [FIX-CATALOG-GATE] Static import â€” these are two pure, synchronous,
...
// takes â€” that path is now the exact same shape it was before WA Catalog
...
async function handleQuestionAction({ session, message, business, tenant, isInteractive }) {
  ...
  // A fresh tap (button/list reply) has no real question yet â€” its `message`
  ...
  // No mode-specific QUESTION flow â€” use the generic AI question handler.
  await updateSession(session.customerPhone, session.tenantId, {
    currentFlow: 'ENQUIRY', step: 'AWAITING_QUESTION',
  });
  if (typedQuestion) {
    // Answer the real question right now instead of waiting for the next
    // message â€” mirrors the AWAITING_QUESTION handling webhookController
  ...
  return {
    type: 'text',
    body: 'â“ What would you like to know? Type your question below.',
  };
}
...
// â†’ ELECTRONICS, CAKE_CUSTOMIZATION â†’ BAKERY, SKINCARE_ADVICE â†’ COSMETICS,
// WALKIN â†’ SALON/BARBERSHOP, ENQUIRY â†’ SERVICES/GENERAL) â€” but the intents
...
// "does this work with my phone" (COMPATIBILITY_CHECK â†’ ENQUIRY, only
// registered for SERVICES/GENERAL), previously hit startFlow()'s "no
// handler" branch and got a flat "âš ï¸ This option is not available" dead end
// instead of any answer â€” for words that look exactly like an ordinary
...
async function startFlowOrAnswerQuestion({ flowName, session, message, business, tenant, isInteractive }) {
  ...
  logger.warn(`[ModuleRegistry] ${mode} has no ${flowName} flow â€” falling back to Q&A`, { flowName, mode });
...
export async function registerAllModules() {
  // â”€â”€ Shared booking flow (all modules that book) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Restaurant â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Retail (dedicated) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Delivery (dedicated) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const { handleDeliveryOrder } = await import('../../modules/delivery/flows/index.js');
  registerFlow('DELIVERY', 'ORDER', handleDeliveryOrder);

  // â”€â”€ Bakery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Salon / Barbershop (dedicated flows â€” not generic wrappers) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const {
    handleSalonBooking,
    handleSalonWalkIn,
  ...
  // Walk-in queue (salon/barbershop only â€” no date/time needed)
  ...
  // â”€â”€ Fashion â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const { handleFashionOrder } = await import('../../modules/fashion/flows/index.js');
  registerFlow('FASHION', 'ORDER', handleFashionOrder);

  // â”€â”€ Cosmetics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // Electronics customers asking "â“ Ask a Question" should land in tech Q&A.
  registerFlow('ELECTRONICS', 'QUESTION',     handleSpecRequest);

  // â”€â”€ Services (dedicated) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ General (dedicated) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Action handlers (module-registered) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // override (see [CATALOG-REG-1] there)" â€” but no such override existed here.
  ...
  // "ðŸ” Order Food" tap open with WA Catalog â€” it always silently went straight
  ...
  // hasSellableProducts(business)` â€” two synchronous field/array checks, zero
  ...
  // startFlow({ flowName: 'ORDER' }) call that ran here before â€” View Menu
  ...
  // requested â€” not one path with a conditional buried inside it:
  //
  //   PATH A â€” tenant has no WA Catalog configured (the default, and every
  ...
  //   startFlow({ flowName: 'ORDER' }) â€” the identical single call this
  ...
  //
  //   PATH B â€” tenant HAS WA Catalog enabled and configured with sellable
  ...
  //   startFlow('ORDER') call PATH A uses â€” WA Catalog can never become a
  ...
  registerAction('START_ORDER', async ({ session, message, business, tenant, intent }) => {
    ...
    // [CATALOG-FIRST] A generic browsing-intent message â€” "I want to order",
    // "I want food", "I want to see your menu", "show me the menu", etc. â€”
    ...
    // matches on substrings â€” a generic word like "food" can accidentally
    // substring-match a real menu item name (e.g. "Seafood Platter") and get
    // treated as a specific product request (single match â†’ silently added
    // to cart, or multiple matches â†’ "which one would you like?"), so the
    ...
    // decision below â€” nothing else can ever come first for them.
    ...
    if (isGenericBrowseIntent) {
      ...
      if (offered) return null; // WA Catalog message already dispatched directly â€” nothing further to send
      return startFlow({ flowName: 'ORDER', session: { ...session, orderChannel: 'menu' }, business, tenant });
    }
    ...
    if (parsedDirect?.ambiguous && parsedDirect.candidates?.length) {
      ...
      return {
        type: 'buttons',
        body: `Which one would you like?\n\n${parsedDirect.candidates.map(item => `â€¢ *${item.name}*`).join('\n')}`,
    ...
    if (lines.length > 0) {
      ...
      if (!handoff) {
        ...
        // startFlow('ORDER') unconditionally â€” the one order-start path in
    ...
    // the customer typed â€” and this block would then tell them "I couldn't
    ...
    // content word survives) means the request was genuinely incomplete â€”
    ...
    // PATH A â€” no WA Catalog for this tenant. Old-version behavior, verbatim.
    ...
    if (directProductText.length >= 3 && !explicitOrderTap && !isFillerOnlyLeftover) {
      ...
      // BUTTON â€” the catalog itself never appeared until the customer spent
      ...
      // the catalog must be sent immediately here too â€” the explanatory text
      ...
      if (catalogReady) {
        ...
        if (fallback) {
          ...
          // the real text/list ORDER menu â€” that IS the "here's what we have"
          // answer, so send it instead of a redundant "couldn't find" notice.
          return fallback;
        }
        // Catalog was dispatched successfully as its own message â€” follow up
        // with the explanation, minus the now-redundant "Browse Catalog"
        // button (the catalog is already on-screen above this message).
        return {
          type: 'buttons',
          body: `I couldn't find *${directProductText.slice(0, 50)}* in our current products â€” take a look at the catalog above, or check the spelling and try again.`,
          buttons: [
    ...
    // [ORDER-CHANNEL] Customer chose Browse Catalog earlier â€” keep them on catalog
    ...
    // PATH B â€” WA Catalog is actually configured for this tenant.
  ...
  registerAction('START_BOOKING', async ({ session, message, business, tenant }) => {
    ...
    // separate START_ORDER handler above, which doesn't help this one) â€”
  ...
  // WALKIN action â€” salon/barbershop walk-in queue (no date/time needed).
  ...
  // so any tenant's customer can type one of these â€” not just SALON/
  ...
  // instead of "âš ï¸ This option is not available".
  ...
  // [FIX-F] SPEC_REQUEST action was never registered â€” unknown action fell through to
  ...
  // modes "fall back gracefully" â€” they didn't; startFlow() had no such fallback
  ...
  // COMPARE â€” side-by-side product comparison (Electronics only).
  // Button id: 'COMPARE' on welcome screen and fallback buttons â€” unlike the
  ...
  // WARRANTY â€” warranty + after-sales enquiry (Electronics only).
  // Can be triggered by typing "warranty", "repair", "return" etc., or future button.
  // [FIX-STARTFLOW-FALLBACK] Those same keywords aren't mode-gated â€” falls back
  ...
  // action too (see intentEngine.js intentToAction map) â€” so a customer in
  ...
  // [FIX-3] QUESTION action â€” routes the QUESTION button tap to the mode-specific
  ...
  // is now in FLOW_PASSTHROUGH_IDS so it reaches route() â†’ ACTION_REGISTRY â†’ here.
  ...
  // returned "âš ï¸ This option is not available right now." â€” a broken UX for any
  // customer tapping "â“ Ask a Question" in those modes. The fix: check whether a
  // mode-specific QUESTION flow exists FIRST; if not, fall back to the generic ENQUIRY
  // flow (sets currentFlow=ENQUIRY / step=AWAITING_QUESTION â†’ AI answers the question).
  ...
  // correct for a genuine "â“ Ask a Question" / "â“ Ask Another" BUTTON tap â€”
  ...
  // same QUESTION action (see intentEngine.js QUESTION â†’ 'QUESTION' mapping) â€”
  ...
  // [FIX-8] ABOUT action â€” delegates directly to the mode handler via route()'s
  ...
  registerAction('REPEAT_ORDER', async ({ session, message, business, tenant }) => {
    ...
    if (lastItem) {
      ...
      // NAME (Order.item is a plain string) â€” writing that straight into session
      ...
      // name-only stub â€” with an explicit price-uncertainty notice â€” only when
      ...
      const priceNotice = !fullItem
        ? `\n\nâš ï¸ We couldn't confirm the current price for this item â€” we'll follow up with the exact total before your order is finalised.`
        : '';
      return {
        type: 'buttons',
        body: `ðŸ” *Repeat your last order*\n\nYou previously ordered *${lastItem}*.${priceNotice}\n\nHow many would you like this time?\n\n_(Enter a number or word â€” e.g. *1*, *2*, *three*)_`,
  ...
  logger.info('[Registry] All modules registered âœ“');
}
