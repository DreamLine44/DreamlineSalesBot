...
const parseOrdinalDayPhrase = (lower, now) => {
  // [FEAT-NLU-BOOKING-PREFILL] Explicit "next month" qualifier on a digit
  // ordinal ("the 15th of next month", "22nd of next month") always targets
  // next calendar month — distinct from the bare "the 15th" case below, which
  // targets the day in THIS month (rolling to next month only if it's already
  // passed). Must run first so an explicit "next month" is never silently
  // reinterpreted as "this month, rolled forward" if the two ever overlap.
  const nextMonthOrdinal = lower.match(/^(?:on\s+the\s+|the\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:day\s+)?of\s+next\s+month$/);
  if (nextMonthOrdinal) {
    const day = parseInt(nextMonthOrdinal[1], 10);
    const d = toUtcMidnight(now.getUTCFullYear(), now.getUTCMonth() + 1, day);
...
const parseRelativeMonthDay = (lower, now) => {
  ...
  // [FEAT-NLU-BOOKING-PREFILL] "day" made optional in all three patterns below —
  // "the first of next month" (no "day") is at least as natural a phrasing as
  // "the first day of next month", and was previously unmatched, silently
  // falling all the way through tryParseDate() to return null.
...
export const looksLikeDate = (input) => {
  ...
  if (/\bnext\s+month/i.test(s) && /\b(first|1st|day|\d{1,2}(?:st|nd|rd|th)?)\b/i.test(s)) return true;
...
async function parseDateWithAI(raw, tz) {
  ...
  try {
    const { parseBookingDate } = await import('../extraction/groqProvider.js');
