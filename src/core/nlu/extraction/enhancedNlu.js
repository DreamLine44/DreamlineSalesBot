...
import { findBestMatch } from '../resolution/matchEngine.js';
import {
...
export async function classifyMessageEnhanced({ message, business, session, validIntents }) {
  ...
  try {
    const { classifyMessageStructured } = await import('./groqProvider.js');
...
async function legacyClassify({ message, validIntents, mode }) {
  try {
    const { classifyIntent } = await import('./groqProvider.js');
