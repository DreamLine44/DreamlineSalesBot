/**
 * core/nlu/classification/intentEngine.js
 *
...
async function classifyWithAI({ message, business, session }) {
  ...
  try {
    const { classifyMessageEnhanced, isEnhancedNluEnabled } = await import('../extraction/enhancedNlu.js');
  ...
  try {
    const { classifyIntent } = await import('../extraction/groqProvider.js').catch(() => ({ classifyIntent: null }));
