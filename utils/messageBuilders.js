...
export function buildWelcomeUI(business) {
  ...
  //
  // [PROFESSIONAL-UX] Default welcome is clean and business-like.
  // No "Hi there!", no excessive emoji. Business name is bolded. One clear CTA.
  // Owners can override via businessConfig.labels.welcomeMessage.
  const rawBody = getLabel(business, 'welcomeMessage') || getLabel(business, 'welcome') ||
...
export function buildCancelUI(business) {
  ...
  // [PROFESSIONAL-UX] Clean, business-like cancellation acknowledgement.
  // No "No worries!" informality — just a clear confirmation and a warm return offer.
