 ...
 *             no-opping.
 * [FIX-BIZ-5] getBusinessConfig now joins the Tenant document so whatsapp.phoneNumberId,
 *             whatsapp.connected, and status are included in the response. Without this,
 *             the frontend always showed "not connected" even when credentials were saved.
 */
...
export async function getBusinessConfig(req, res) {
  try {
    ...
    // [FIX-BIZ-5] Join Tenant doc so whatsapp status reaches the frontend
    const [biz, tenantDoc] = await Promise.all([
      BusinessConfig.findOne({ tenantId }).lean(),
      Tenant.findById(tenantId).select('whatsapp status').lean(),
    ]);
    if (!biz) return res.status(404).json({ error: 'Not found' });
    const whatsapp = tenantDoc?.whatsapp ? {
      phoneNumberId: tenantDoc.whatsapp.phoneNumberId || null,
      connected:     tenantDoc.whatsapp.connected     || false,
      phone:         tenantDoc.whatsapp.phone         || null,
    } : null;
    res.json({ business: { ...biz, whatsapp, status: tenantDoc?.status || biz.status } });
  } catch (err) {
