...
// const VALID_MODES         = ['ORDER', 'BOOKING', 'BOTH'];          // legacy mode field
// const VALID_BUSINESS_MODES = ['RESTAURANT', 'SALON', 'RETAIL'];    // businessMode field
...
//               hours: {
//                 enabled: false,
//                 open:    8,   // integer hour (0–23), e.g. 8 = 08:00
//                 close:   18,  // integer hour (0–23), e.g. 18 = 18:00
//               },
//               payment: {
...
//     // Validate mode if provided (legacy field: ORDER / BOOKING / BOTH)
...
//         data[field] = UPPERCASE_FIELDS.has(field) ? req.body[field].toUpperCase() : req.body[field];
//       }
//     }
...
//           ? `Mode: ${business?.businessMode || 'RESTAURANT'}, Menu items: ${business?.menu?.length || 0}` // [FIX-7] was business?.mode (legacy) — businessMode is the canonical v15 field
 ...
 *
 * FIX (v5.0.1):
 * [FIX-1] setupBusiness: Mongoose ValidationError and MongoServerError (E11000) are
 *         now caught explicitly in the catch block and returned as clean 400 responses
 *         instead of propagating as unhandled TypeErrors. This eliminates the
 *         "TypeError: Cannot read properties of undefined (reading 'length')" crash
 *         that occurred when runValidators:true triggered validation on the upsert
 *         insert path before the BusinessConfig model fix (required:false) was applied.
 * [FIX-2] adminPhone validation now accepts E.164 format (+2203532423) in addition
 *         to bare digit strings. The old regex /^\d{7,15}$/ rejected the "+" prefix.
 * [FIX-3] setupBusiness catch block now safely accesses err properties with guards
 *         so no secondary crash can occur inside the error handler path.
 */
...
export const registerBusiness = async (req, res) => {
  try {
    ...
    logger.error('[Onboarding] registerBusiness error', { err: String(err?.message) });
    res.status(500).json({ success: false, message: 'Server error. Please try again.' });
  }
};
...
export const setupBusiness = async (req, res) => {
  try {
    ...
    // Validate legacy mode field if provided (ORDER / BOOKING / BOTH)
    if (req.body.mode && !VALID_MODES.includes(req.body.mode?.toUpperCase())) {
      return res.status(400).json({
        success: false,
        message: `Invalid mode "${req.body.mode}". Valid values: ${VALID_MODES.join(', ')}. To set business type use "businessMode" instead.`,
    ...
    // [FIX-2] Validate adminPhone — accept E.164 (+2203532423) or bare digits (2203532423)
    if (req.body.adminPhone !== undefined) {
      const phone   = String(req.body.adminPhone).trim();
      const isE164  = /^\+[1-9]\d{6,14}$/.test(phone);
      const isDigit = /^\d{7,15}$/.test(phone.replace(/[\s\-]/g, ''));
      if (!isE164 && !isDigit) {
        return res.status(400).json({
          success: false,
          message: 'adminPhone must be in E.164 format (e.g. +2203532423) or digits only (e.g. 2203532423).',
        });
      }
    }
    ...
    for (const field of ALLOWED) {
      if (req.body[field] !== undefined) {
        data[field] = UPPERCASE_FIELDS.has(field)
    ...
    // [FIX-1] Upsert by tenantId.
    // runValidators:true is removed — Mongoose v8 triggers required validation
    // on the upsert INSERT path even when the field is not in $set, which causes
    // a ValidationError (and a secondary TypeError in its internal handler) when
    // phoneNumberId is not yet set (pre–WhatsApp-connection step).
    // Field-level validation above already guards all user-supplied values.
    const business = await BusinessConfig.findOneAndUpdate(
      { tenantId: tenant._id },
      { $set: data },
      { new: true, upsert: true, runValidators: false }  // ← runValidators:false (FIXED)
    );

    logger.info('[Onboarding] Business configured', { tenantId: String(tenant._id) });
    ...
    // [FIX-3] Safe error handling — never call .length or access nested properties
    // on err without guards. Mongoose ValidationError and MongoServerError (E11000)
    // are both handled explicitly so they return clean 400s instead of 500s.
    const errMsg  = String(err?.message || 'Unknown error');
    const errName = String(err?.name    || 'Error');
    const errCode = err?.code;

    // Mongoose ValidationError (e.g. enum mismatch, min/max violation)
    if (errName === 'ValidationError') {
      const fields = Object.values(err.errors || {}).map(e => String(e?.message || e));
      logger.warn('[Onboarding] setupBusiness validation error', { fields });
      return res.status(400).json({
        success: false,
    ...
    // MongoDB duplicate key error (E11000) — e.g. two upserts race on phoneNumberId
    if (errCode === 11000) {
      logger.warn('[Onboarding] setupBusiness duplicate key', { errMsg });
      return res.status(409).json({
        success: false,
        message: 'A business config for this account already exists. Use PUT to update it.',
      });
    }

    logger.error('[Onboarding] setupBusiness error', { err: errMsg });
    res.status(500).json({ success: false, message: 'Server error. Please try again.' });
  }
};
...
export const connectWhatsApp = async (req, res) => {
  try {
    ...
    logger.info('[Onboarding] WhatsApp connected', { tenantId: String(tenant._id), phoneNumberId });
    ...
    logger.error('[Onboarding] connectWhatsApp error', { err: String(err?.message) });
    res.status(500).json({ success: false, message: 'Server error. Please try again.' });
  }
};
  ...
  try {
    ...
    const steps = [
      ...
      },
      {
        ...
        detail: hasName && hasAdmin
          ? `Mode: ${business?.businessMode || business?.mode || 'RESTAURANT'}, Menu items: ${business?.menu?.length || 0}`
          : 'POST /register/business with name, adminPhone, menu, etc.',
        subChecks: {
          name:        hasName,
          adminPhone:  hasAdmin,
    ...
    logger.error('[Onboarding] getStatus error', { err: String(err?.message) });
    res.status(500).json({ success: false, message: 'Server error' });
  }
};